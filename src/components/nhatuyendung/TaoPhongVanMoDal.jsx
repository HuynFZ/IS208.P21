...
export default function TaoPhongVanModal({ 
  isOpen, 
  onClose, 
  onSuccess,
  isUpdate,
  ...
  // Reset form khi modal đóng và điền dữ liệu khi cập nhật
  useEffect(() => {
  ...
  // Load tin tuyển dụng khi modal mở
  useEffect(() => {
    if (isOpen) {
      loadTinTuyenDungs();
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = isUpdate 
        ? `/api/nhatuyendung/phongvan/${maPhongVan}`
        : '/api/nhatuyendung/phongvan/taophongvan';
      
      const method = isUpdate ? 'PUT' : 'POST';
      const res = await axios[method.toLowerCase()](url, form);

      if (res.data.success) {
        showToast(`${isUpdate ? 'Cập nhật' : 'Tạo'} phỏng vấn thành công`, { type: 'success' });
        onSuccess();
        onClose();
      }
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
      <div className="bg-white rounded-lg p-6 w-[500px] max-h-[80vh] overflow-y-auto">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">
          {isUpdate ? 'Cập nhật phỏng vấn' : 'Tạo phỏng vấn mới'}
        </h3>
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm 
                  focus:border-teal-500 focus:ring-teal-500 bg-white text-gray-900"
                placeholder="Mô tả chi tiết về buổi phỏng vấn..."
              />
            </div>
          </div>
          ...
          <div className="mt-6 flex justify-end space-x-3">
            ...
            >
              {loading ? 'Đang xử lý...' : isUpdate ? 'Cập nhật' : 'Tạo mới'}
            </button>
          </div>
        </form>
      </div>
